...
const manualImportRoute: FastifyPluginCallback = (app, _opts, done) => {
  app.get("/manual-import", async (request, reply) => {
    ...
    if (!query.downloadId && !query.folder) {
      ...
      return {
        message:
          "Provide either downloadId or folder to fetch manual import candidates.",
      };
    }
    ...
    try {
      const candidates = await fetchManualImportCandidates(
        fetcher,
        query.service,
        options,
      );
      ...
      const status =
        error instanceof ManualImportError ? error.statusCode : 502;
      const message =
        error instanceof Error
          ? error.message
          : "Unable to fetch manual import candidates.";
      reply.status(status);
      return { message };
    }
  });
  ...
  app.post("/manual-import", async (request, reply) => {
    ...
    const body = manualImportSubmissionSchema.parse(
      request.body ?? {},
    ...
    try {
      await submitManualImportCommand(
        fetcher,
        body.service,
        body.files,
