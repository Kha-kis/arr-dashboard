...
const normalizeSearchLanguages = (value: unknown): Array<{ id: number; name: string }> | undefined => {
  if (!Array.isArray(value)) {
    return undefined;
  }
...
﻿const normalizeIndexer = (
  ...
  const result = prowlarrIndexerSchema.safeParse({
    ...
    supportsRss:
      typeof indexer.supportsRss !== "undefined" ? toBoolean(indexer.supportsRss, false) : undefined,
    supportsSearch:
      typeof indexer.supportsSearch !== "undefined" ? toBoolean(indexer.supportsSearch, true) : undefined,
    supportsRedirect:
      typeof indexer.supportsRedirect !== "undefined" ? toBoolean(indexer.supportsRedirect, false) : undefined,
...
export const registerSearchRoutes: FastifyPluginCallback = (app, _opts, done) => {
  ...
  app.get("/search/indexers/:instanceId/:indexerId", async (request, reply) => {
    ...
    try {
      ...
      if (!details) {
        reply.status(502);
        return searchIndexerDetailsResponseSchema.parse({
          indexer: buildIndexerDetailsFallback(instance.id, instance.label, instance.baseUrl, indexerId),
        });
      }
  ...
  app.put("/search/indexers/:instanceId/:indexerId", async (request, reply) => {
    ...
    const payload: SearchIndexerUpdateRequest = searchIndexerUpdateRequestSchema.parse(request.body ?? {});
    ...
    try {
      ...
      request.log.error({ err: error, instance: instance.id, indexerId: indexerIdValue }, "prowlarr indexer update failed");
      reply.status(502);
      return searchIndexerDetailsResponseSchema.parse({
        indexer: buildIndexerDetailsFallback(instance.id, instance.label, instance.baseUrl, indexerIdValue),
      });
    }
    ...
    try {
      ...
      request.log.error({ err: error, instance: instance.id, indexerId: indexerIdValue }, "prowlarr indexer fetch after update failed");
    }
  ...
  app.post("/search/indexers/test", async (request, reply) => {
    ...
    const instance = await app.prisma.serviceInstance.findFirst({
      ...
      where: { userId: request.currentUser.id, enabled: true, service: "PROWLARR", id: payload.instanceId },
    ...
    if (!instance) {
      ...
      return searchIndexerTestResponseSchema.parse({ success: false, message: "Indexer instance not found" });
    ...
    try {
      ...
      request.log.error({ err: error, instance: instance.id, indexerId: payload.indexerId }, "prowlarr indexer test failed");
  ...
  app.post("/search/grab", async (request, reply) => {
    ...
    try {
      ...
      return { success: false, message: error instanceof Error ? error.message : "Failed to grab release" };
