 ...
 */
export const registerIndexerRoutes: FastifyPluginCallback = (
  app,
  _opts,
  done,
) => {
    ...
    for (const instance of instances) {
      ...
      try {
        ...
        request.log.error(
          ...
          "prowlarr indexers fetch failed",
        );
  ...
  app.get("/search/indexers/:instanceId/:indexerId", async (request, reply) => {
    ...
    try {
      const details = await fetchProwlarrIndexerDetails(
        fetcherInstance,
        instance,
        indexerId,
      );
  ...
  app.put("/search/indexers/:instanceId/:indexerId", async (request, reply) => {
    ...
    const payload: SearchIndexerUpdateRequest =
      searchIndexerUpdateRequestSchema.parse(request.body ?? {});
    ...
    const bodyIndexer = {
      ...originalIndexer,
      id:
        typeof originalIndexer.id === "number"
          ? originalIndexer.id
          : indexerIdValue,
    ...
    try {
      const updated = await fetchProwlarrIndexerDetails(
        fetcherInstance,
        instance,
        indexerIdValue,
      );
  ...
  app.post("/search/indexers/test", async (request, reply) => {
    ...
    try {
      ...
      return searchIndexerTestResponseSchema.parse({
        ...
        message:
          error instanceof Error ? error.message : "Failed to test indexer",
      });
    }
  });
