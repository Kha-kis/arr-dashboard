...
const deriveQueueActions = (item: any): QueueActionCapabilities => {
  ...
  const downloadId = toStringValue(item?.downloadId ?? item?.guid ?? item?.sourceId ?? item?.data?.downloadId);
  ...
  const appearsCompleted = status.includes("completed") || status.includes("downloadclientunavailable");
...
const normalizeQueueItem = (item: any, service: "sonarr" | "radarr"): Omit<QueueItem, "instanceId" | "instanceName"> => {
  const rawId =
    item.id ??
    item.queueId ??
    item.queueItemId ??
  ...
  const id = typeof rawId === "number" || typeof rawId === "string" ? rawId : Math.random().toString(36);
  ...
  const normalized: Omit<QueueItem, "instanceId" | "instanceName"> = {
    ...
    series: item.series && typeof item.series === "object" ? {
      id: toNumber(item.series.id),
      title: toStringValue(item.series.title) ?? undefined,
    } : undefined,
    movie: item.movie && typeof item.movie === "object" ? {
    ...
    downloadClient: toStringValue(item.downloadClient ?? item.downloadClientName ?? item.data?.downloadClient),
...
const normalizeHistoryItem = (item: any, service: "sonarr" | "radarr" | "prowlarr"): HistoryItem => {
  const rawId =
    item.id ??
    item.eventId ??
    item.downloadId ??
    ...
    item.guid ??
    Math.random().toString(36);
  const normalizedId = typeof rawId === "number" || typeof rawId === "string" ? rawId : Math.random().toString(36);
  ...
  const downloadId =
    ...
    (typeof normalizedId === "number" || typeof normalizedId === "string" ? String(normalizedId) : undefined);
  ...
  return {
    ...
    downloadClient:
      toStringValue(item.downloadClient ?? dataObj.downloadClient ?? dataObj.downloadClientName),
    indexer:
      prowlarrSource ??
      toStringValue(item.indexer ?? dataObj.indexer ?? dataObj.indexerName),
    protocol:
      toStringValue(item.protocol ?? item.downloadProtocol ?? dataObj.protocol),
    date:
      toStringValue(item.date ?? item.eventDate ?? item.eventDateUtc ?? item.created ?? item.timestamp),
    reason:
      toStringValue(item.reason ?? dataObj.reason ?? item.error ?? dataObj.message ?? dataObj.statusMessage),
...
const normalizeCalendarItem = (item: any, service: "sonarr" | "radarr"): CalendarItem => {
  ...
  const seriesTitle =
    service === "sonarr"
      ? toStringValue(item.series?.title) ?? toStringValue(item.seriesTitle) ?? toStringValue(item.title)
      : undefined;
  const episodeTitle =
    service === "sonarr"
      ? toStringValue(item.title) ??
      ...
      : undefined;
  const movieTitle = service === "radarr" ? toStringValue(item.title) ?? toStringValue(item.originalTitle) : undefined;

  const seriesId = service === "sonarr" ? toNumber(item.seriesId ?? item.series?.id) : undefined;
  const seriesSlug = service === "sonarr" ? toStringValue(item.series?.titleSlug ?? item.titleSlug ?? item.series?.path) : undefined;
  ...
  const movieSlug = service === "radarr" ? toStringValue(item.movie?.titleSlug ?? item.titleSlug ?? item.movie?.path) : undefined;
  const tmdbId = toNumber(item.tmdbId ?? item.tmdbid ?? item.movie?.tmdbId ?? item.series?.tmdbId);
  const imdbId = toStringValue(item.imdbId ?? item.imdbid ?? item.movie?.imdbId ?? item.series?.imdbId);
...
const fetchQueueItems = async (
  ...
  const query = service === "sonarr"
    ? "?pageSize=1000&includeUnknownSeriesItems=true"
    : "?pageSize=1000";
...
const fetchHistoryItems = async (
  ...
  const totalRecords =
    payload && typeof payload === "object" && !Array.isArray(payload) && typeof payload.totalRecords === "number"
      ? payload.totalRecords
      : payload && typeof payload === "object" && !Array.isArray(payload) && typeof payload.total === "number"
        ? payload.total
        : records.length;
...
const dashboardRoute: FastifyPluginCallback = (app, _opts, done) => {
    ...
    if (body.action === "manualImport") {
      ...
      const search = new URLSearchParams({
        ...
        changeCategory: String(body.changeCategory),
            ...
            await fetcher(`${queueApiPath(body.service)}/${queueId}?${search.toString()}`, { method: "DELETE" });
      if (body.search) {
        try {
          await triggerQueueSearch(fetcher, body.service, body.searchPayload);
        } catch (error) {
          request.log.error({ err: error, queueId, service: body.service }, "queue search trigger failed");
        }
      }
    }
  ...
  app.post("/dashboard/queue/bulk", async (request, reply) => {
    ...
    if (body.action === "retry") {
      await Promise.all(queueIds.map((id) => fetcher(`${queueApiPath(body.service)}/${id}/retry`, { method: "POST" })));
    } else {
  ...
  app.get("/dashboard/history", async (request, reply) => {
    ...
    for (const instance of instances) {
      ...
      try {
        ...
        const { items, totalRecords } = await fetchHistoryItems(fetcher, service, 1, 10000, startDate, endDate);
        const enriched = items.map((item) => ({
          ...item,
          instanceId: instance.id,
          instanceName: instance.label,
  ...
  app.get("/dashboard/calendar", async (request, reply) => {
    ...
    const defaultEnd = new Date(Date.UTC(startDate.getUTCFullYear(), startDate.getUTCMonth() + 1, 0));
    ...
    for (const instance of instances) {
      ...
      if (service === "sonarr") {
        try {
          ...
          request.log.error({ err: error, instance: instance.id }, "sonarr statistics fetch failed");
          sonarrInstances.push({ instanceId: instance.id, instanceName: instance.label, data: emptySonarrStatistics });
        }
        continue;
      }
      ...
      if (service === "radarr") {
        try {
          ...
          request.log.error({ err: error, instance: instance.id }, "radarr statistics fetch failed");
          radarrInstances.push({ instanceId: instance.id, instanceName: instance.label, data: emptyRadarrStatistics });
        }
        continue;
      }
      ...
      try {
        ...
        request.log.error({ err: error, instance: instance.id }, "prowlarr statistics fetch failed");
        prowlarrInstances.push({ instanceId: instance.id, instanceName: instance.label, data: emptyProwlarrStatistics });
      }
    }
