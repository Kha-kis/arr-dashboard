...
const normalizeRejections = (
  rejections: unknown,
): ManualImportCandidate["rejections"] => {
...
const normalizeEpisodes = (
  episodes: unknown,
): ManualImportCandidateSonarr["episodes"] => {
...
const normalizeSeries = (
  series: unknown,
): ManualImportCandidateSonarr["series"] => {
...
const normalizeMovie = (
  movie: unknown,
): ManualImportCandidateRadarr["movie"] => {
...
const mapCandidate = (
  ...
  const base = {
    ...
    customFormats: Array.isArray(item?.customFormats)
      ? item.customFormats
      : undefined,
...
export const fetchManualImportCandidates = async (
  ...
  try {
    ...
    throw new ManualImportError(
      "ARR returned an invalid manual import payload.",
    );
  }
  ...
  const candidates = rawItems
    ...
    .filter(
      (candidate): candidate is ManualImportCandidate => candidate !== null,
    );
...
export const collectAutoManualImportFiles = (
  ...
  for (const candidate of candidates) {
    const humanName =
      candidate.relativePath ?? candidate.name ?? candidate.path;
...
const buildCommandFiles = (
  ...
  return files.map((file, index) => {
    ...
    if (!downloadId) {
      throw new ManualImportError(
        `File at index ${index} is missing a download identifier.`,
      );
    }
    ...
    const base: Record<string, unknown> = {
      ...
      indexerFlags:
        typeof file.indexerFlags === "number" ? file.indexerFlags : 0,
      downloadId,
    };
...
export const autoImportByDownloadId = async (
  ...
  if (candidates.length === 0) {
    throw new ManualImportError(
      "ARR did not provide any importable files for this download.",
    );
  }

  const { files, skipped } = collectAutoManualImportFiles(
    service,
    candidates,
    downloadId,
  );
  ...
  if (files.length === 0) {
    const detail =
      skipped.length > 0 ? skipped.slice(0, 3).join("; ") : undefined;
    throw new ManualImportError(
      detail
