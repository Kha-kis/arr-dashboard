...
const normalizeImages = (
  ...
  for (const raw of images as RemoteImages[]) {
    ...
    if (
      (coverType === "fanart" || coverType === "background") &&
      !result.fanart
    ) {
...
const mergeInstanceState = (
  ...
  const existingIndex = states.findIndex(
    (entry) => entry.instanceId === state.instanceId,
  );
  if (existingIndex >= 0) {
...
const createInstanceState = (
  ...
  hasFile: toBoolean(
    payload?.hasFile ?? payload?.hasMovieFile ?? payload?.hasEpisodeFile,
  ),
...
const normalizeLookupResult = (
  ...
  const runtime = toNumber(
    raw?.runtime ?? raw?.runtimeMinutes ?? raw?.runtimeHours,
  );
      ...
      const rootFolders = Array.isArray(rootFoldersRaw)
        ? rootFoldersRaw.reduce<
          ...
          >((acc, folder: unknown) => {
            ...
            acc.push({
              id:
                toNumber(folder?.id) ?? toStringValue(folder?.id) ?? undefined,
              path,
    ...
    try {
      ...
      const seasons = Array.isArray(remote?.seasons)
        ? remote.seasons.map((season: unknown) => ({
            ...
            monitored:
              seriesPayload.seasonFolder === false
                ? false
                : (seriesPayload.monitored ?? true),
          }))
        : [];
