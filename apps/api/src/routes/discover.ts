...
const normalizeImages = (images: unknown, baseUrl?: string): { poster?: string; fanart?: string; banner?: string } => {
...
const normalizeLookupResult = (
  ...
  return {
    ...
    studio,
    runtime,
    ratings: ratingsValue !== undefined || ratingsVotes !== undefined ? { value: ratingsValue, votes: ratingsVotes } : undefined,
  ...
  app.get("/discover/options", async (request, reply) => {
    ...
    try {
      ...
      const qualityProfiles = Array.isArray(qualityProfilesRaw)
        ? qualityProfilesRaw
            .map((profile: any) => ({ id: toNumber(profile?.id), name: toStringValue(profile?.name) }))
            .filter((profile): profile is { id: number; name: string } => typeof profile.id === "number" && typeof profile.name === "string")
        : [];

      const rootFolders = Array.isArray(rootFoldersRaw)
        ? rootFoldersRaw.reduce<Array<{ id?: number | string; path: string; accessible?: boolean; freeSpace?: number }>>((acc, folder: any) => {
      ...
      if (service === "sonarr") {
        try {
          ...
          request.log.warn({ err: error, instance: instance.id }, "failed to load language profiles");
        }
      }
    ...
    try {
      if (service === "radarr") {
        ...
        const body = {
          ...
          minimumAvailability:
            moviePayload.minimumAvailability ?? toStringValue(remote?.minimumAvailability) ?? "announced",
          addOptions: {
      ...
      const seasons = Array.isArray(remote?.seasons)
        ? remote.seasons.map((season: any) => ({
            ...
            monitored: seriesPayload.seasonFolder === false ? false : seriesPayload.monitored ?? true,
          }))
        : [];

      const languageProfileId = seriesPayload.languageProfileId ?? toNumber(remote?.languageProfileId);
      ...
      const body = {
        ...
        titleSlug: toStringValue(remote?.titleSlug) ?? slugify(seriesPayload.title ?? remote.title ?? "series"),
