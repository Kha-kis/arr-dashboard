...
const serviceUpdateSchema = servicePayloadSchema.partial({
  label: true,
  baseUrl: true,
  apiKey: true,
  service: true,
...
const servicesRoute: FastifyPluginCallback = (app, _opts, done) => {
  ...
  app.put("/services/:id", async (request, reply) => {
    ...
    if (payload.label) {updateData.label = payload.label;}
    if (payload.baseUrl) {updateData.baseUrl = payload.baseUrl;}
    if (typeof payload.enabled === "boolean") {updateData.enabled = payload.enabled;}
    if (typeof payload.isDefault === "boolean") {updateData.isDefault = payload.isDefault;}
    if (payload.service) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (contentType?.includes("text/html")) {
          details = "Received HTML instead of JSON. The base URL or API path might be incorrect. Ensure base URL includes the full path (e.g., http://localhost:7878 not http://localhost:7878/radarr)";
        }
      ...
      if (!contentType?.includes("application/json")) {
        return reply.status(200).send({
          ...
          details: "Received HTML instead of JSON. Check if the base URL is correct and includes any URL base if configured in the service (e.g., http://localhost:7878 for root, or http://localhost/radarr if using a URL base).",
        });
      }
      ...
      if (error.name === "TimeoutError" || error.code === "ETIMEDOUT") {
        ...
        details = "The service did not respond within 5 seconds. Check if the service is running and the base URL is correct.";
      } else if (error.code === "ECONNREFUSED") {
        errorMessage = "Connection refused";
        details = "Could not connect to the service. Verify the base URL and that the service is running.";
