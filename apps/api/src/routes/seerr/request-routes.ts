...
const requestIdParams = z.object({ instanceId: z.string().min(1), requestId: z.coerce.number().int().positive() });

const listRequestsQuery = z.object({
	take: z.coerce.number().int().min(1).max(100).default(20),
	skip: z.coerce.number().int().min(0).default(0),
	filter: z.enum(["all", "approved", "available", "pending", "processing", "unavailable", "failed"]).default("all"),
