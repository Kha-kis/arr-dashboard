...
export const emptySonarrStatistics: SonarrStatistics =
  sonarrStatisticsSchema.parse({
    totalSeries: 0,
    monitoredSeries: 0,
    continuingSeries: 0,
    endedSeries: 0,
...
export const emptyRadarrStatistics: RadarrStatistics =
  radarrStatisticsSchema.parse({
    totalMovies: 0,
    monitoredMovies: 0,
    downloadedMovies: 0,
    missingMovies: 0,
...
export const emptyProwlarrStatistics: ProwlarrStatistics =
  prowlarrStatisticsSchema.parse({
    totalIndexers: 0,
    activeIndexers: 0,
    pausedIndexers: 0,
    totalQueries: 0,
...
export const fetchSonarrStatistics = async (
  ...
  const series =
    (await safeRequestJson<unknown[]>(fetcher, "/api/v3/series")) ?? [];
  const diskspace =
    (await safeRequestJson<unknown[]>(fetcher, "/api/v3/diskspace")) ?? [];
  const health =
    (await safeRequestJson<unknown[]>(fetcher, "/api/v3/system/health")) ?? [];
  const cutoffUnmet =
  ...
  for (const entry of series) {
    ...
    const episodesTotal =
      toNumber(stats.totalEpisodeCount ?? stats.episodeCount) ?? 0;
    ...
    if (episodesWithFile > 0) {
      const qualityProfileId = toNumber(
        (entry as { qualityProfileId?: unknown }).qualityProfileId,
      );

      // Look up the profile name
      if (
        qualityProfileId !== undefined &&
        profileIdToName.has(qualityProfileId)
      ) {
        const profileName = profileIdToName.get(qualityProfileId);
        if (profileName !== undefined) {
          qualityBreakdown[profileName] =
            (qualityBreakdown[profileName] ?? 0) + episodesWithFile;
        }
      } else {
        qualityBreakdown.Unknown =
          (qualityBreakdown.Unknown ?? 0) + episodesWithFile;
      }
    }
  }
  ...
  const diskUsagePercent =
    diskTotals.total > 0
      ? clampPercentage((diskUsed / diskTotals.total) * 100)
      : 0;
  ...
  return sonarrStatisticsSchema.parse({
    ...
    downloadedPercentage:
      totalEpisodes > 0
        ? clampPercentage((downloadedEpisodes / totalEpisodes) * 100)
        : 0,
    cutoffUnmetCount,
    qualityBreakdown,
    averageEpisodeSize,
...
export const fetchRadarrStatistics = async (
  ...
  const movies =
    (await safeRequestJson<unknown[]>(fetcher, "/api/v3/movie")) ?? [];
  const diskspace =
    (await safeRequestJson<unknown[]>(fetcher, "/api/v3/diskspace")) ?? [];
  const health =
    (await safeRequestJson<unknown[]>(fetcher, "/api/v3/system/health")) ?? [];
  ...
  for (const movie of movies) {
    ...
    if ((movie as { hasFile?: unknown }).hasFile) {
      ...
      const sizeOnDisk =
        toNumber((movie as { sizeOnDisk?: unknown }).sizeOnDisk) ?? 0;
      totalFileSize += sizeOnDisk;

      // Use quality profile to categorize
      const qualityProfileId = toNumber(
        (movie as { qualityProfileId?: unknown }).qualityProfileId,
      );
      ...
      if (
        ...
        if (profileName !== undefined) {
          qualityBreakdown[profileName] =
            (qualityBreakdown[profileName] ?? 0) + 1;
        }
      } else {
  ...
  const averageMovieSize =
    downloadedMovies > 0 ? totalFileSize / downloadedMovies : undefined;
  ...
  return radarrStatisticsSchema.parse({
    ...
    downloadedPercentage:
      monitoredMovies > 0
        ? clampPercentage((downloadedMovies / monitoredMovies) * 100)
        : 0,
    cutoffUnmetCount,
    qualityBreakdown,
    averageMovieSize,
...
export const fetchProwlarrStatistics = async (
  ...
  const indexers =
    (await safeRequestJson<unknown[]>(fetcher, "/api/v1/indexer")) ?? [];
  const health =
    (await safeRequestJson<unknown[]>(fetcher, "/api/v1/health")) ?? [];
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    const totalRecords = history.totalRecords ?? 0;
  ...
  const activeIndexers = indexers.filter(
    (entry) => entry && entry.enable !== false,
  ).length;
  ...
  const indexerStatsMap = new Map<
    string,
    { queries: number; grabs: number; successful: number }
  >();
  ...
  for (const record of historyRecords) {
    ...
    const eventType =
      toStringValue(
        (record as { eventType?: unknown }).eventType,
      )?.toLowerCase() ?? "";
    const successful =
      (record as { successful?: unknown }).successful !== false;
  ...
  const normalizedStats: ProwlarrIndexerStat[] = Array.from(
    indexerStatsMap.entries(),
  )
    .map(([name, stats]) => {
      const successRate =
        stats.queries > 0 ? (stats.successful / stats.queries) * 100 : 0;
  ...
  const totalQueries = sumNumbers(
    normalizedStats.map((entry) => entry.queries),
  );
  ...
  for (const stat of normalizedStats) {
    const querySuccessCount = Math.round(
      (stat.queries * stat.successRate) / 100,
    );
  ...
  const grabRate =
    totalQueries > 0 ? clampPercentage((totalGrabs / totalQueries) * 100) : 0;
  ...
  const topIndexers = normalizedStats
...
export const aggregateSonarrStatistics = (
  ...
  const totals = instances.reduce(
    (acc, entry) => {
      ...
      if (data.qualityBreakdown) {
        for (const [profileName, count] of Object.entries(
          data.qualityBreakdown,
        )) {
          acc.qualityBreakdown[profileName] =
            (acc.qualityBreakdown[profileName] ?? 0) + count;
        }
      }
  ...
  const downloadedPercentage =
    totals.totalEpisodes > 0
      ? clampPercentage(
          (totals.downloadedEpisodes / totals.totalEpisodes) * 100,
        )
      : 0;
  const diskUsagePercent =
    totals.diskTotal > 0
      ? clampPercentage((totals.diskUsed / totals.diskTotal) * 100)
      : 0;
  const averageEpisodeSize =
    totals.totalFiles > 0
      ? totals.totalFileSize / totals.totalFiles
      : undefined;
...
export const aggregateRadarrStatistics = (
  ...
  const downloadedPercentage =
    totals.monitoredMovies > 0
      ? clampPercentage(
          (totals.downloadedMovies / totals.monitoredMovies) * 100,
        )
      : 0;
  const diskUsagePercent =
    totals.diskTotal > 0
...
export const aggregateProwlarrStatistics = (
  ...
  const averageResponseTime =
    totals.responseTimes.length > 0
      ...
      : undefined;
  const grabRate =
    totals.totalQueries > 0
      ? clampPercentage((totals.totalGrabs / totals.totalQueries) * 100)
      : 0;
  ...
  const indexers = Array.from(aggregatedIndexers.values())
    .map((aggregate) =>
      prowlarrIndexerStatSchema.parse({
        ...
        successRate: clampPercentage(
          aggregate.successRateSum / Math.max(aggregate.count, 1),
        ),
      }),
    )
    .sort((a, b) => b.queries - a.queries)
