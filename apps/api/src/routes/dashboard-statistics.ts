...
export const fetchSonarrStatistics = async (
  ...
  const cutoffUnmet = (await safeRequestJson<any>(fetcher, "/api/v3/wanted/cutoff?page=1&pageSize=1")) ?? {};
  ...
  const averageEpisodeSize = downloadedEpisodes > 0 ? totalFileSize / downloadedEpisodes : undefined;
  ...
  const diskUsagePercent = diskTotals.total > 0 ? clampPercentage((diskUsed / diskTotals.total) * 100) : 0;
...
export const fetchProwlarrStatistics = async (
  ...
  for (let page = 1; page <= maxPages; page++) {
    const history = (await safeRequestJson<any>(
      fetcher,
  ...
  const activeIndexers = indexers.filter((entry) => entry && entry.enable !== false).length;
  ...
  for (const record of historyRecords) {
    ...
    if (eventType === "grab" || eventType === "releaseGrabbed" || eventType === "grabbed" || eventType === "releasegrab") {
      stats.grabs += 1;
    }
  }
...
export const aggregateSonarrStatistics = (
  ...
  const downloadedPercentage = totals.totalEpisodes > 0 ? clampPercentage((totals.downloadedEpisodes / totals.totalEpisodes) * 100) : 0;
  const diskUsagePercent = totals.diskTotal > 0 ? clampPercentage((totals.diskUsed / totals.diskTotal) * 100) : 0;
  const averageEpisodeSize = totals.totalFiles > 0 ? totals.totalFileSize / totals.totalFiles : undefined;
...
export const aggregateRadarrStatistics = (
  ...
  const downloadedPercentage = totals.monitoredMovies > 0 ? clampPercentage((totals.downloadedMovies / totals.monitoredMovies) * 100) : 0;
  const diskUsagePercent = totals.diskTotal > 0 ? clampPercentage((totals.diskUsed / totals.diskTotal) * 100) : 0;
  const averageMovieSize = totals.totalFiles > 0 ? totals.totalFileSize / totals.totalFiles : undefined;
...
export const aggregateProwlarrStatistics = (
  ...
  const averageResponseTime = totals.responseTimes.length > 0
    ? totals.responseTimes.reduce((sum, value) => sum + value, 0) / totals.responseTimes.length
    : undefined;
  const grabRate = totals.totalQueries > 0 ? clampPercentage((totals.totalGrabs / totals.totalQueries) * 100) : 0;

  const aggregatedIndexers = totals.indexers.reduce(
    (map: Map<string, { displayName: string; queries: number; grabs: number; successRateSum: number; count: number }>, entry) => {
    ...
    new Map<string, { displayName: string; queries: number; grabs: number; successRateSum: number; count: number }>(),
  );

  const indexers = Array.from(aggregatedIndexers.values())
    .map((aggregate) => prowlarrIndexerStatSchema.parse({
