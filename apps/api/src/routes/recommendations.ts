...
const recommendationsRoute: FastifyPluginCallback = (app, _opts, done) => {
  app.get("/recommendations", async (request, reply) => {
    ...
    if (!user?.encryptedTmdbApiKey || !user?.tmdbEncryptionIv) {
      ...
      return reply.send({
        message:
          "TMDB API key not configured. Please add your TMDB API key in Settings.",
      });
    }
    ...
    try {
      let tmdbData:
        ...
        | TMDBResponse<TMDBTVShow>
        | undefined;

      if (parsed.mediaType === "movie") {
        switch (parsed.type) {
          case "trending":
            tmdbData = await getTrendingMovies(tmdbApiKey, "week", parsed.page);
            break;
          case "popular":
            tmdbData = await getPopularMovies(tmdbApiKey, parsed.page);
            break;
          case "top_rated":
            tmdbData = await getTopRatedMovies(tmdbApiKey, parsed.page);
            break;
          case "upcoming":
            tmdbData = await getUpcomingMovies(tmdbApiKey, parsed.page);
            break;
          default:
            tmdbData = await getTrendingMovies(tmdbApiKey, "week", parsed.page);
        }
        ...
        const items: RecommendationItem[] = (
          ...
          backdropUrl:
            getTMDBImageUrl(movie.backdrop_path, "original") || undefined,
      ...
      switch (parsed.type) {
        case "trending":
          tmdbData = await getTrendingTV(tmdbApiKey, "week", parsed.page);
          break;
        case "popular":
          tmdbData = await getPopularTV(tmdbApiKey, parsed.page);
          break;
        case "top_rated":
          tmdbData = await getTopRatedTV(tmdbApiKey, parsed.page);
          break;
        case "airing_today":
          tmdbData = await getAiringTodayTV(tmdbApiKey, parsed.page);
          break;
        default:
          tmdbData = await getTrendingTV(tmdbApiKey, "week", parsed.page);
      }
      ...
      const items: RecommendationItem[] = (
        ...
        backdropUrl:
          getTMDBImageUrl(show.backdrop_path, "original") || undefined,
