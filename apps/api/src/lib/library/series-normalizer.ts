  ...
  const seasons = value
    .map((entry: any) => {
      ...
      const missingEpisodeCount =
        monitored === false ? 0 : missingEpisodeCountRaw;
...
export const buildSeriesItem = (
  ...
  const episodeFileCount =
    toNumber(stats?.episodeFileCount ?? raw?.episodeFileCount) ?? 0;

  return {
    id:
      toNumber(raw?.id) ?? toStringValue(raw?.id) ?? Math.random().toString(36),
    ...
    statistics: {
      ...
      monitoredSeasons: Array.isArray(raw?.seasons)
        ? raw.seasons.filter((season: any) => toBoolean(season?.monitored))
            .length
        : undefined,
