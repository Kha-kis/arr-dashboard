	...
	for (const [tmdbId, showRatingKeys] of selectedShows) {
		try {
			...
			for (const copies of episodesByCoordinate.values()) {
				...
				const watched = watchCount > 0 || watchData.some((data) => data.eventCount > 0);
				const observedUsers = [...new Set(watchData.flatMap((data) => [...data.users]))];
				const episodeAttributionComplete = watchData.every((data) => data.attributionComplete);
				...
				const lastWatchedAt = latestWatchedAt > 0 ? new Date(latestWatchedAt * 1000) : null;
				...
				const effectiveWatchedByUsers =
					historyComplete && episodeAttributionComplete
						? observedUsers
						: [...new Set([...(compatibleExistingState?.watchedByUsers ?? []), ...observedUsers])];
