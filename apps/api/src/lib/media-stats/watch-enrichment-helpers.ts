 ...
 *
 * Pure functions for aggregating PlexCache entries into
...
/**
 * Aggregate PlexCache entries into WatchEnrichmentItems for each unique
 * key, optionally filtered by a specific user. (Tautulli as a second
 * source was removed in 3.0 — ADR-0007.)
 */
export function aggregateWatchEnrichment(
	...
	for (const [key, { tmdbId, mediaType }] of uniqueKeys) {
		...
		if (plexMatches.length === 0) continue;
