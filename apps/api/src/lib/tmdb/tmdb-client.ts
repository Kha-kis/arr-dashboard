...
async function tmdbFetch<T>(endpoint: string, apiKey: string, page = 1): Promise<T> {
...
async function fetchMultiplePages<T extends { results: unknown[] }>(
	...
	const requests = Array.from({ length: pages }, (_, i) => tmdbFetch<T>(endpoint, apiKey, i + 1));
...
async function fetchSinglePage<T>(endpoint: string, apiKey: string, page: number): Promise<T> {
...
export async function getPopularMovies(apiKey: string, page = 1): Promise<TMDBResponse<TMDBMovie>> {
	// Fetch 3 pages at once to account for library filtering
	const pagesToFetch = 3;
	const startPage = (page - 1) * pagesToFetch + 1;
	...
	const responses = await Promise.all(
		Array.from({ length: pagesToFetch }, (_, i) =>
			fetchSinglePage<TMDBResponse<TMDBMovie>>("/movie/popular", apiKey, startPage + i),
		),
	);
...
export async function getPopularTV(apiKey: string, page = 1): Promise<TMDBResponse<TMDBTVShow>> {
	// Fetch 3 pages at once to account for library filtering
	const pagesToFetch = 3;
	const startPage = (page - 1) * pagesToFetch + 1;
	...
	const responses = await Promise.all(
		Array.from({ length: pagesToFetch }, (_, i) =>
			fetchSinglePage<TMDBResponse<TMDBTVShow>>("/tv/popular", apiKey, startPage + i),
		),
	);
...
export async function getTopRatedMovies(
	...
	const responses = await Promise.all(
		Array.from({ length: pagesToFetch }, (_, i) =>
			fetchSinglePage<TMDBResponse<TMDBMovie>>("/movie/top_rated", apiKey, startPage + i),
		),
	);
...
export async function getTopRatedTV(apiKey: string, page = 1): Promise<TMDBResponse<TMDBTVShow>> {
	// Fetch 3 pages at once to account for library filtering
	const pagesToFetch = 3;
	const startPage = (page - 1) * pagesToFetch + 1;
	...
	const responses = await Promise.all(
		Array.from({ length: pagesToFetch }, (_, i) =>
			fetchSinglePage<TMDBResponse<TMDBTVShow>>("/tv/top_rated", apiKey, startPage + i),
		),
	);
...
export async function getUpcomingMovies(
	...
	const responses = await Promise.all(
		Array.from({ length: pagesToFetch }, (_, i) =>
			fetchSinglePage<TMDBResponse<TMDBMovie>>("/movie/upcoming", apiKey, startPage + i),
		),
	);
...
export async function getAiringTodayTV(
	...
	const responses = await Promise.all(
		Array.from({ length: pagesToFetch }, (_, i) =>
			fetchSinglePage<TMDBResponse<TMDBTVShow>>("/tv/airing_today", apiKey, startPage + i),
		),
	);
