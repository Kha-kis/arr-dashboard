  ...
  for (const entry of value) {
    ...
    if (typeof entry === "object") {
      ...
      const name = toStringValue(
        record.name ?? record.language ?? record.label ?? record.value,
      );
...
export const normalizeSearchResult = (
  ...
  const id =
    ...
    toStringValue(
      raw.downloadUrl ?? raw.downloadLink ?? raw.downloadURI ?? raw.link,
    );
  ...
  const candidate = {
    ...
    indexer: indexerName,
    indexerId,
    categories: parseNumberArray(
      raw.categories ?? raw.category ?? raw.categoryIds,
    ),
    seeders: seeders ?? undefined,
    leechers: leechers ?? undefined,
    peers: peers ?? undefined,
    grabs:
      toNumber(raw.grabs ?? raw.historyCount ?? raw.downloaded) ?? undefined,
    protocol: parseProtocol(toStringValue(raw.protocol)),
    publishDate:
      parseDateValue(raw.publishDate ?? raw.firstSeen ?? raw.updated) ??
      undefined,
    age: toNumber(raw.age) ?? undefined,
    ageHours: toNumber(raw.ageHours) ?? undefined,
    ageDays: toNumber(raw.ageDays) ?? undefined,
    downloadClient:
      toStringValue(raw.downloadClient ?? raw.downloadClientName) ?? undefined,
...
export const normalizeIndexer = (
  ...
  const name =
    toStringValue(indexer.name) ?? toStringValue(indexer.title) ?? "Indexer";
  if (typeof id !== "number") {
    return null;
  }
...
export const normalizeIndexerStats = (
  ...
  const parsed = prowlarrIndexerStatsSchema.safeParse({
    status:
      toStringValue(stats.status) ?? toStringValue(stats.state) ?? undefined,
    ...
    lastCheck:
      toStringValue(stats.lastCheck) ??
      toStringValue(stats.lastExecution) ??
      undefined,
...
export const normalizeIndexerDetails = (
  ...
  const detail = {
    ...
    instanceUrl: instance.baseUrl,
    implementationName:
      toStringValue(raw.implementationName) ??
      toStringValue(raw.implementation) ??
      undefined,
    ...
    fields: Array.isArray(raw.fields)
      ? raw.fields
          .map((field) =>
            normalizeIndexerField(field as Record<string, unknown>),
          )
