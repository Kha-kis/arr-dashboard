...
export const fetchHistoryItems = async (
  ...
  const response = await fetcher(
    `${historyApiPath(service)}?${params.toString()}`,
  );
  const payload = await response.json();
  ...
  const items = records.map((raw: unknown) =>
...
export const fetchCalendarItems = async (
  ...
  const response = await fetcher(
    `${calendarApiPath(service)}?${params.toString()}`,
  );
  const payload = await response.json();
  const records = Array.isArray(payload)
    ? payload
    : Array.isArray(payload?.records)
      ? payload.records
      : [];
  const normalized = records.map((raw: unknown) =>
