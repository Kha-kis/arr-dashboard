 ...
 */
export const calendarApiPath = (service: "sonarr" | "radarr") =>
  "/api/v3/calendar";
...
export const normalizeCalendarItem = (
  ...
  const { local: airDate, utc: airDateUtc } = selectCalendarDates(
    item,
    service,
  );
  ...
  const seriesId =
    service === "sonarr"
      ? toNumber(anyItem.seriesId ?? anyItem.series?.id)
      : undefined;
  const seriesSlug =
    service === "sonarr"
      ? toStringValue(
          anyItem.series?.titleSlug ??
            anyItem.titleSlug ??
            anyItem.series?.path,
        )
      : undefined;
  const episodeId =
    service === "sonarr"
      ? toNumber(anyItem.episodeId ?? anyItem.id)
      : undefined;
  const movieId =
    service === "radarr" ? toNumber(anyItem.movieId ?? anyItem.id) : undefined;
  const movieSlug =
    service === "radarr"
      ? toStringValue(
          anyItem.movie?.titleSlug ?? anyItem.titleSlug ?? anyItem.movie?.path,
        )
      : undefined;
  const tmdbId = toNumber(
    anyItem.tmdbId ??
      anyItem.tmdbid ??
      anyItem.movie?.tmdbId ??
      anyItem.series?.tmdbId,
  );
  const imdbId = toStringValue(
    anyItem.imdbId ??
      anyItem.imdbid ??
      anyItem.movie?.imdbId ??
      anyItem.series?.imdbId,
  );
  const seriesStatus =
    service === "sonarr" ? toStringValue(anyItem.series?.status) : undefined;
  const status = toStringValue(
    anyItem.status ?? anyItem.movie?.status ?? anyItem.series?.status,
  );
  ...
  return {
    ...
    status,
    seasonNumber:
      service === "sonarr" ? toNumber(anyItem.seasonNumber) : undefined,
    episodeNumber:
      service === "sonarr" ? toNumber(anyItem.episodeNumber) : undefined,
    airDate,
    airDateUtc,
    ...
    genres:
      toStringArray(anyItem.genres) ?? toStringArray(anyItem.series?.genres),
 ...
 */
export const compareCalendarItems = (
  a: CalendarItem,
  b: CalendarItem,
): number => {
