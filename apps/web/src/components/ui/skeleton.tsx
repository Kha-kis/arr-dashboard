 ...
 */
const skeletonVariants = cva("rounded-lg bg-muted", {
	variants: {
		animation: {
			pulse: "animate-pulse",
 ...
 * ```
 */
function Skeleton({ className, animation, themed, ...props }: SkeletonProps) {
		...
		<div className={cn("space-y-2", className)} role="status" aria-label="Loading text">
			{Array.from({ length: lines }).map((_, i) => (
				<Skeleton
					...
					animation={animation}
					themed={themed}
					className={cn("h-4", i === 0 ? "w-full" : i === lines - 1 ? "w-1/2" : "w-3/4")}
	...
	return (
		...
		>
			...
			{Array.from({ length: rows }).map((_, rowIndex) => (
				<div key={`row-${rowIndex}`} className="border-b border-border/30 last:border-0 px-4 py-4">
					<div className="flex gap-4">
