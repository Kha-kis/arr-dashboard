	...
	return (
		<section className="space-y-4">
			<div className="flex items-center justify-between">
				<h2 className="text-2xl font-semibold text-foreground">{serviceType}</h2>
				<button
					type="button"
					onClick={onRefresh}
					disabled={refreshing || isRefreshPending}
					className="inline-flex items-center gap-2 rounded-lg bg-primary/20 px-4 py-2 text-sm font-medium text-foreground transition hover:bg-primary/30 disabled:opacity-50"
				>
			...
			{statuses.length === 0 ? (
				<div className="rounded-xl border border-border bg-card p-8 text-center">
					<p className="text-muted-foreground">No cache entries for {serviceType}</p>
					<button
						type="button"
						onClick={onRefresh}
						disabled={refreshing || isRefreshPending}
