...
export const useHistoryData = (
  ...
  const instanceOptions = useMemo(
    () => extractInstanceOptions(instances),
    [instances],
  );

  const statusOptions = useMemo(
    () => extractStatusOptions(allAggregated),
    [allAggregated],
  );
  ...
  const filteredItems = useMemo(() => {
    ...
    return allAggregated.filter((item) => {
      if (
        ...
        item.service !== filters.serviceFilter
      ) {
        return false;
      }
      if (
        filters.instanceFilter !== "all" &&
        item.instanceId !== filters.instanceFilter
      ) {
        return false;
      }
      ...
      if (
        ...
        currentStatus !== filters.statusFilter
      ) {
        return false;
      }
      if (term.length > 0) {
  ...
  const serviceSummary = useMemo(
    () => createServiceSummary(allAggregated),
    [allAggregated],
  );

  const statusSummary = useMemo(
    () => createStatusSummary(filteredItems),
    [filteredItems],
  );
