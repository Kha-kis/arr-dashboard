...
export function useLibraryActions(): LibraryActions {
	 ...
	 */
	const handleSeasonMonitor = useCallback(
		async (series: LibraryItem, seasonNumber: number, nextMonitored: boolean) => {
	...
	const handleSeasonSearch = useCallback(
		async (series: LibraryItem, seasonNumber: number) => {
			if (series.service !== "sonarr") {
	 ...
	 */
	const handleAlbumMonitor = useCallback(
		async (artist: LibraryItem, albumId: number, nextMonitored: boolean) => {
			if (artist.service !== "lidarr") {
			...
			try {
				...
				const message = getErrorMessage(error, "Unknown error");
				toast.error(
					`Failed to ${nextMonitored ? "enable" : "disable"} album monitoring: ${message}`,
				);
			} finally {
	...
	const handleAlbumSearch = useCallback(
		async (artist: LibraryItem, albumIds: number[]) => {
			if (artist.service !== "lidarr") {
	 ...
	 */
	const handleBookMonitor = useCallback(
		async (author: LibraryItem, bookId: number, nextMonitored: boolean) => {
			if (author.service !== "readarr") {
			...
			try {
				...
				const message = getErrorMessage(error, "Unknown error");
				toast.error(
					`Failed to ${nextMonitored ? "enable" : "disable"} book monitoring: ${message}`,
				);
			} finally {
	...
	const handleBookSearch = useCallback(
		async (author: LibraryItem, bookIds: number[]) => {
			if (author.service !== "readarr") {
