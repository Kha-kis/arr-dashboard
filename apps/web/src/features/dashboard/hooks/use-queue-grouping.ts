 ...
 */
export const filterItemsForAction = (items: QueueItem[], action: QueueAction): QueueItem[] => {
	if (action === "retry") {
...
const createGroupSummary = (key: string, items: QueueItem[]): SummaryRow => {
	...
	const uniqueStatuses = new Set(
		items.map((item) => item.status?.trim()).filter((status): status is string => Boolean(status)),
	);
	const statusLabel =
