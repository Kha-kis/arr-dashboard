...
# 3. Bootstrap Jellyfin and Seerr
# Seerr 3.x requires a real first-admin login before its protected setup APIs
# are available. Complete Jellyfin's supported setup UI first, then use real
# Seerr admin/requester sessions in the TypeScript fixture.
...
log "Completing Jellyfin setup through its supported web wizard..."
pnpm exec playwright test fixtures/jellyfin-setup.setup.ts \
  --config=playwright.integration.config.ts --project=setup --no-deps

log "Initializing Seerr with authenticated admin and requester sessions..."
SEERR_BOOTSTRAP_OUTPUT=$(
  RADARR_API_KEY="$RADARR_API_KEY" \
  SEERR_EXTERNAL_URL="$SEERR_URL" \
  pnpm --filter @arr/api exec tsx "$SCRIPT_DIR/fixtures/bootstrap-seerr.ts"
)
echo "$SEERR_BOOTSTRAP_OUTPUT"
SEERR_TEST_USER_ID=$(echo "$SEERR_BOOTSTRAP_OUTPUT" | grep -oP '^SEERR_TEST_USER_ID=\K[0-9]+$')
[ -n "$SEERR_TEST_USER_ID" ] || fail "Seerr bootstrap did not return a requester id"
