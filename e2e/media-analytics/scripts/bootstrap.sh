#!/usr/bin/env bash

set -euo pipefail
umask 077

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)"
source "${SCRIPT_DIR}/common.sh"
source "${SCRIPT_DIR}/wait-for-services.sh"

STATE_DIR="${HARNESS_DIR}/.state"
RUNTIME_ENV="${STATE_DIR}/runtime.env"
BOOTSTRAP_JSON="${STATE_DIR}/bootstrap.json"
TRACEARR_COOKIE_JAR="${STATE_DIR}/tracearr-cookie.jar"
DASHBOARD_COOKIE_JAR="${STATE_DIR}/dashboard-cookie.jar"

local_plex_instruction() {
	cat >&2 <<'EOF'
Local Plex mode cannot complete the real provider connection flow with this
upstream version. Re-run once with an invocation-only claim:
PLEX_CLAIM=claim-... pnpm e2e:media-analytics:reset. The claim is not
persisted in harness state or retained in Plex container metadata.
EOF
}

load_runtime_env() {
	local mode owner
	if [[ ! -f "$RUNTIME_ENV" ]]; then
		echo "runtime state was not generated" >&2
		return 1
	fi
	mode="$(stat -c '%a' "$RUNTIME_ENV")"
	owner="$(stat -c '%u' "$RUNTIME_ENV")"
	if [[ "$mode" != "600" || "$owner" != "$(id -u)" ]]; then
		echo "refusing unsafe runtime state file permissions" >&2
		return 1
	fi
	# The generator writes only fixed shell assignments with hexadecimal values.
	# shellcheck disable=SC1090
	source "$RUNTIME_ENV"
	export TRACEARR_JWT_SECRET TRACEARR_COOKIE_SECRET TRACEARR_DB_PASSWORD TAUTULLI_API_KEY
}

curl_json() {
	curl -fsS -H 'Accept: application/json' "$@"
}

require_json() {
	local expression="$1"
	local payload="$2"
	jq -e "$expression" >/dev/null <<< "$payload"
}

read_plex_machine_identifier() {
	local plex_url="$1"
	local attempts="${PLEX_IDENTITY_ATTEMPTS:-30}"
	local interval_seconds="${PLEX_IDENTITY_INTERVAL_SECONDS:-2}"
	local attempt plex_identity plex_machine_identifier

	for ((attempt = 1; attempt <= attempts; attempt++)); do
		plex_identity="$(curl_json "${plex_url}/identity" 2>/dev/null || true)"
		plex_machine_identifier="$(jq -er '.MediaContainer.machineIdentifier // empty' <<< "$plex_identity" 2>/dev/null || true)"
		if [[ -n "$plex_machine_identifier" ]]; then
			printf '%s' "$plex_machine_identifier"
			return 0
		fi
		if ((attempt < attempts)); then
			sleep "$interval_seconds"
		fi
	done

	echo "Plex identity did not include a machine identifier after $attempts attempts" >&2
	return 1
}

read_plex_server_token() {
	local attempts="$1"
	local interval_seconds="${PLEX_TOKEN_INTERVAL_SECONDS:-2}"
	local attempt token

	for ((attempt = 1; attempt <= attempts; attempt++)); do
		token="$(compose exec -T plex sh -lc \
			"sed -n 's/.*PlexOnlineToken=\"\\([^\"]*\\)\".*/\\1/p' '/config/Library/Application Support/Plex Media Server/Preferences.xml'" \
			2>/dev/null | tr -d '\r\n' || true)"
		if [[ -n "$token" ]]; then
			printf '%s' "$token"
			return 0
		fi
		if ((attempt < attempts)); then
			sleep "$interval_seconds"
		fi
	done

	return 1
}

discover_plex_library_agent() {
	local plex_url="$1"
	local plex_token="$2"
	local agents agent

	agents="$(curl -fsS -H "X-Plex-Token: ${plex_token}" "${plex_url}/system/agents?mediaType=1")"
	agent="$(sed -n 's/.*identifier="\([^"]*\)".*/\1/p' <<< "$agents" \
		| awk '$0 == "tv.plex.agents.movie" { print; found = 1; exit } !fallback { fallback = $0 } END { if (!found) print fallback }')"
	if [[ -z "$agent" ]]; then
		echo "Plex did not report a supported movie library agent" >&2
		return 1
	fi
	printf '%s' "$agent"
}

ensure_plex_library() {
	local plex_url="$1"
	local plex_token="$2"
	local agent libraries

	libraries="$(curl_json -H "X-Plex-Token: ${plex_token}" "${plex_url}/library/sections")"
	if require_json 'any((.MediaContainer.Directory // [])[]; .title == "Synthetic Test" and any((.Location // [])[]; .path == "/data"))' "$libraries"; then
		return 0
	fi
	if require_json 'any((.MediaContainer.Directory // [])[]; .title == "Synthetic Test")' "$libraries"; then
		echo "Plex already has a Synthetic Test library with an unexpected location" >&2
		return 1
	fi

	agent="$(discover_plex_library_agent "$plex_url" "$plex_token")"
	if ! curl -fsS -G -X POST "${plex_url}/library/sections" \
		-H "X-Plex-Token: ${plex_token}" \
		--data-urlencode 'name=Synthetic Test' \
		--data-urlencode 'type=movie' \
		--data-urlencode "agent=${agent}" \
		--data-urlencode 'scanner=Plex Movie' \
		--data-urlencode 'language=en-US' \
		--data-urlencode 'location=/data' >/dev/null; then
		echo "Plex authenticated library setup failed" >&2
		return 1
	fi
}

ensure_dashboard_service() {
	local dashboard_api_url="$1"
	local service="$2"
	local label="$3"
	local base_url="$4"
	local api_key="$5"
	local response matches match_count service_id

	response="$(curl_json -b "$DASHBOARD_COOKIE_JAR" "${dashboard_api_url}/api/services")"
	matches="$(jq -c \
		--arg label "$label" \
		--arg service "$service" \
		--arg baseUrl "$base_url" \
		'[.services[]? | select(.label == $label or (.service == $service and .baseUrl == $baseUrl))]' \
		<<< "$response")"
	match_count="$(jq -r 'length' <<< "$matches")"

	if [[ "$match_count" == "0" ]]; then
		response="$(curl_json -b "$DASHBOARD_COOKIE_JAR" -X POST "${dashboard_api_url}/api/services" \
			-H 'Content-Type: application/json' \
			--data "$(jq -cn --arg label "$label" --arg baseUrl "$base_url" --arg apiKey "$api_key" --arg service "$service" '{label:$label,baseUrl:$baseUrl,apiKey:$apiKey,service:$service}')")"
		jq -er '.service.id' <<< "$response"
		return 0
	fi

	if [[ "$match_count" != "1" ]] || ! jq -e \
		--arg label "$label" \
		--arg service "$service" \
		--arg baseUrl "$base_url" \
		'.[0].label == $label and .[0].service == $service and .[0].baseUrl == $baseUrl and .[0].hasApiKey == true' \
		>/dev/null <<< "$matches"; then
		echo "arr-dashboard service collision for ${label}" >&2
		return 1
	fi

	service_id="$(jq -er '.[0].id' <<< "$matches")"
	# Tracearr rotates its disposable public API key during each bootstrap. Refresh
	# the stored credential without creating another service instance.
	curl_json -b "$DASHBOARD_COOKIE_JAR" -X PUT "${dashboard_api_url}/api/services/${service_id}" \
		-H 'Content-Type: application/json' \
		--data "$(jq -cn --arg apiKey "$api_key" '{apiKey:$apiKey}')" >/dev/null
	printf '%s' "$service_id"
}

assert_dashboard_connection() {
	local dashboard_api_url="$1"
	local service_name="$2"
	local service_id="$3"
	local response

	if ! response="$(curl_json -b "$DASHBOARD_COOKIE_JAR" -X POST "${dashboard_api_url}/api/services/${service_id}/test" -H 'Content-Type: application/json' --data '{}')"; then
		echo "arr-dashboard connection test request failed for ${service_name}" >&2
		return 1
	fi
	if ! require_json '.success == true' "$response"; then
		echo "arr-dashboard connection test failed for ${service_name}" >&2
		return 1
	fi
}

mkdir -p "$STATE_DIR"
chmod 0700 "$STATE_DIR"
require_command flock

# Every checkout controls the same fixed Compose project, so the bootstrap lock
# must live outside the worktree. Keep it in an owner-only runtime directory and
# key it by project and uid so separate checkouts cannot enroll the same stack
# concurrently.
lock_parent="${XDG_RUNTIME_DIR:-/tmp}"
lock_dir="${lock_parent%/}/${COMPOSE_PROJECT}-$(id -u)"
lock_file="${lock_dir}/bootstrap.lock"
if [[ -L "$lock_dir" ]]; then
	echo "refusing unsafe media analytics bootstrap lock directory" >&2
	exit 1
fi
if ! mkdir -m 0700 "$lock_dir" 2>/dev/null && [[ ! -d "$lock_dir" ]]; then
	echo "unable to create media analytics bootstrap lock directory" >&2
	exit 1
fi
if [[ "$(stat -c '%u' "$lock_dir")" != "$(id -u)" || "$(stat -c '%a' "$lock_dir")" != "700" ]]; then
	echo "refusing unsafe media analytics bootstrap lock directory" >&2
	exit 1
fi
if [[ -L "$lock_file" ]]; then
	echo "refusing unsafe media analytics bootstrap lock file" >&2
	exit 1
fi
exec 9>"$lock_file"
chmod 0600 "$lock_file"
if ! flock -n 9; then
	echo "media analytics bootstrap is already running" >&2
	exit 1
fi

claim_cleanup_required=false
cleanup_bootstrap() {
	local exit_status=$?
	trap - EXIT
	rm -f "$TRACEARR_COOKIE_JAR" "$DASHBOARD_COOKIE_JAR"
	if [[ "$claim_cleanup_required" == "true" ]]; then
		unset PLEX_CLAIM
		if ! compose up -d --force-recreate plex >/dev/null 2>&1; then
			echo "warning: failed to scrub the one-time Plex claim from container metadata" >&2
		fi
	fi
	exit "$exit_status"
}

"${SCRIPT_DIR}/generate-secrets.sh"
load_runtime_env
rm -f "$BOOTSTRAP_JSON" "$TRACEARR_COOKIE_JAR" "$DASHBOARD_COOKIE_JAR"
trap cleanup_bootstrap EXIT

if [[ -n "${PLEX_TOKEN:-}" ]]; then
	echo "PLEX_TOKEN is not accepted; the harness reads Plex's server-issued token after claim exchange" >&2
	exit 1
fi

claim_requested=false
if [[ -n "${PLEX_CLAIM:-}" ]]; then
	claim_requested=true
fi
export PLEX_CLAIM

plex_url="http://127.0.0.1:${PLEX_PORT:-32400}"
tautulli_url="http://127.0.0.1:${TAUTULLI_PORT:-38181}"
tracearr_url="http://127.0.0.1:${TRACEARR_PORT:-33000}"
dashboard_url="http://127.0.0.1:${DASHBOARD_PORT:-33030}"
dashboard_api_url="http://127.0.0.1:${DASHBOARD_API_PORT:-33031}"

if [[ "$claim_requested" == "true" ]]; then
	claim_cleanup_required=true
fi
compose up -d plex
wait_for_http "Plex" "${plex_url}/identity" 200 "${READINESS_ATTEMPTS:-30}" "${READINESS_INTERVAL_SECONDS:-2}"
plex_machine_identifier="$(read_plex_machine_identifier "$plex_url")"
export TAUTULLI_PMS_IDENTIFIER="$plex_machine_identifier"
if [[ "$claim_requested" == "true" ]]; then
	unset PLEX_CLAIM
fi

mode="local"
plex_client_token="$PLEX_LOCAL_TOKEN"
plex_token_attempts=1
if [[ "$claim_requested" == "true" ]]; then
	plex_token_attempts="${PLEX_TOKEN_ATTEMPTS:-30}"
fi
plex_server_token="$(read_plex_server_token "$plex_token_attempts" || true)"
if [[ -n "$plex_server_token" ]]; then
	mode="claimed"
	plex_client_token="$plex_server_token"
elif [[ "$claim_requested" == "true" ]]; then
	echo "Plex did not issue a server token after claim exchange" >&2
	exit 1
fi

if [[ "$claim_requested" == "true" ]]; then
	# The claim is needed only by Plex's first start. Recreate the container
	# against the same claimed volume without the one-time credential so it does
	# not remain recoverable from container metadata during the rest of the run.
	compose up -d --force-recreate plex
	wait_for_http "Plex" "${plex_url}/identity" 200 "${READINESS_ATTEMPTS:-30}" "${READINESS_INTERVAL_SECONDS:-2}"
	plex_machine_identifier_after_scrub="$(read_plex_machine_identifier "$plex_url")"
	if [[ "$plex_machine_identifier_after_scrub" != "$plex_machine_identifier" ]]; then
		echo "Plex identity changed while scrubbing the one-time claim" >&2
		exit 1
	fi

	if [[ "$mode" != "claimed" ]]; then
		echo "Plex claim exchange did not produce a claimed server" >&2
		exit 1
	fi
	claim_cleanup_required=false
fi

if [[ "$mode" == "claimed" ]]; then
	ensure_plex_library "$plex_url" "$plex_client_token"
fi

export TAUTULLI_FIRST_RUN_COMPLETE=1
export TAUTULLI_PMS_TOKEN="$plex_client_token"
export TAUTULLI_PMS_IP=plex
export TAUTULLI_PMS_PORT=32400
export TAUTULLI_PMS_URL_MANUAL=1
export TAUTULLI_PMS_SSL=0
export TAUTULLI_API_ENABLED=1
export TAUTULLI_API_KEY

compose up -d tautulli tracearr tracearr-db tracearr-redis arr-dashboard
wait_for_media_analytics_services

tautulli_status="$(curl_json "${tautulli_url}/api/v2?apikey=${TAUTULLI_API_KEY}&cmd=server_status")"
if ! require_json '.response.data.connected == true' "$tautulli_status"; then
	if [[ "$mode" == "local" ]]; then
		local_plex_instruction
	fi
	echo "Tautulli did not report a connected Plex server" >&2
	exit 1
fi

tracearr_setup="$(curl_json "${tracearr_url}/api/v1/setup/status")"
if ! require_json '.requiresClaimCode == false and ((.needsSetup == true and .hasServers == false) or (.needsSetup == false and .hasServers == true))' "$tracearr_setup"; then
	echo "Tracearr did not report a resumable setup state" >&2
	exit 1
fi

if require_json '.hasPasswordAuth == true or .hasServers == true' "$tracearr_setup"; then
	curl_json -c "$TRACEARR_COOKIE_JAR" -X POST "${tracearr_url}/api/v1/auth/sign-in/username" \
		-H 'Content-Type: application/json' \
		--data "$(jq -cn --arg password "$DASHBOARD_ADMIN_PASSWORD" '{username:"media_analytics",password:$password,rememberMe:false}')" >/dev/null
else
	curl_json -c "$TRACEARR_COOKIE_JAR" -X POST "${tracearr_url}/api/v1/auth/sign-up/email" \
		-H 'Content-Type: application/json' \
		--data "$(jq -cn --arg password "$DASHBOARD_ADMIN_PASSWORD" '{username:"media_analytics",email:"media-analytics@example.test",name:"Media Analytics",password:$password}')" >/dev/null
fi
chmod 0600 "$TRACEARR_COOKIE_JAR"

if ! require_json '.hasServers == true' "$tracearr_setup"; then
	if ! curl_json -b "$TRACEARR_COOKIE_JAR" -X POST "${tracearr_url}/api/v1/servers" \
		-H 'Content-Type: application/json' \
		--data "$(jq -cn --arg token "$plex_client_token" '{type:"plex",name:"E2E Plex",url:"http://plex:32400",token:$token}')" >/dev/null; then
		if [[ "$mode" == "local" ]]; then
			local_plex_instruction
		fi
		echo "Tracearr Plex server setup failed" >&2
		exit 1
	fi
fi

tracearr_key_response="$(curl_json -b "$TRACEARR_COOKIE_JAR" -X POST "${tracearr_url}/api/v1/settings/api-key/regenerate")"
tracearr_api_key="$(jq -er '.token | select(startswith("trr_pub_"))' <<< "$tracearr_key_response")"
curl_json "${tracearr_url}/api/v2/public/docs" -H "Authorization: Bearer ${tracearr_api_key}" >/dev/null

dashboard_setup="$(curl_json "${dashboard_api_url}/auth/setup-required")"
if ! require_json '.required | type == "boolean"' "$dashboard_setup"; then
	echo "arr-dashboard setup status was invalid" >&2
	exit 1
fi
if require_json '.required == true' "$dashboard_setup"; then
	curl_json -c "$DASHBOARD_COOKIE_JAR" -X POST "${dashboard_api_url}/auth/register" \
		-H 'Content-Type: application/json' \
		--data "$(jq -cn --arg password "$DASHBOARD_ADMIN_PASSWORD" '{username:"media-analytics",password:$password,rememberMe:false}')" >/dev/null
fi
curl_json -b "$DASHBOARD_COOKIE_JAR" -c "$DASHBOARD_COOKIE_JAR" -X POST "${dashboard_api_url}/auth/login" \
	-H 'Content-Type: application/json' \
	--data "$(jq -cn --arg password "$DASHBOARD_ADMIN_PASSWORD" '{username:"media-analytics",password:$password,rememberMe:false}')" >/dev/null
chmod 0600 "$DASHBOARD_COOKIE_JAR"

plex_service_id="$(ensure_dashboard_service "$dashboard_api_url" plex 'E2E Plex' 'http://plex:32400' "$plex_client_token")"
tautulli_service_id="$(ensure_dashboard_service "$dashboard_api_url" tautulli 'E2E Tautulli' 'http://tautulli:8181' "$TAUTULLI_API_KEY")"
tracearr_service_id="$(ensure_dashboard_service "$dashboard_api_url" tracearr 'E2E Tracearr' 'http://tracearr:3000' "$tracearr_api_key")"

if ! assert_dashboard_connection "$dashboard_api_url" Plex "$plex_service_id"; then
	if [[ "$mode" == "local" ]]; then
		local_plex_instruction
	fi
	exit 1
fi
assert_dashboard_connection "$dashboard_api_url" Tautulli "$tautulli_service_id"
assert_dashboard_connection "$dashboard_api_url" Tracearr "$tracearr_service_id"

cat > "$BOOTSTRAP_JSON" <<EOF
{"schemaVersion":1,"serviceMode":"${mode}","plexIdentityObserved":true,"tautulliConnected":true,"tracearrConnected":true,"arrDashboardConnectionsVerified":true}
EOF
chmod 0600 "$BOOTSTRAP_JSON"
