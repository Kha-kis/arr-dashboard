...
> **Version 2.20.0** — The **qui integration release**. Federates with [autobrr/qui](https://github.com/autobrr/qui) to add a torrent-layer observability arc across the library, Pulse, queue cleaner, and a dedicated `/qui` overview: per-card torrent health and tracker brand icons, server-side torrent-state filtering, sort-by-ratio, a 480 px per-torrent detail drawer with capability-aware actions, MediaInfo quality verification on files, season-grouped torrent panels on series/movies, queue-cleaner last-seed protection, and a qui Activity tab with its own webhook receiver (#475). Reframes cross-seed as an **integrity lens** keyed on tracker health and brand resolution (#492). Two privacy fixes strip raw tracker passkeys from the qui trackers route (#493, closes #491) and from the cross-seed sibling display (#492). Storage Available card now de-duplicates disks shared across instances and shows "N disks across M instances" (#490, closes #486). Library Cleanup remembers rejected items so they stop re-proposing (#482, closes #474). Hunting grab detection no longer requires the `eventType` filter (#479, closes #472). `MALLOC_ARENA_MAX=2` set as container default with `rssMB/heapTotalMB` surfaced in heap-monitor payload to distinguish JS leaks from glibc allocator behavior (#478). Plus query-string percent-encoding fix (#476, closes #470), heap-monitor kill switch (#477, closes #471), CodeQL-flagged qui debug-endpoint env-gate (#485), and the usual dependency sweeps.
...
[![Support me on Ko-fi](https://ko-fi.com/img/githubbutton_sm.svg)](https://ko-fi.com/khak1s)
