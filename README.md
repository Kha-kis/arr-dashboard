...
> **Version 2.23.0** — The **torrent payload safeguards release**. Queue Cleaner can now enforce an opt-in, per-instance torrent file-extension allowlist through qui. Mixed torrents containing even one unexpected file are removed in full through Sonarr or Radarr when the existing safety gates permit cleanup, while unavailable manifest metadata fails closed and is retried later (#565, #612). PostCSS is updated to 8.5.20 to resolve GHSA-6g55-p6wh-862q (#611).
