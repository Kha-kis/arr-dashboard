...
> **Version 2.22.0** — The **authenticated services & hardened maintenance release**. Service instances behind authenticated reverse proxies can now use optional encrypted HTTP Basic Auth credentials, including protected qui deployments (#600). New-library notifications catch imports completed between polling ticks (#601, #543), Library Cleanup reads Sonarr's flat ratings correctly (#604), and the 2.x dependency, formatting, lint, and supply-chain security backlog is resolved (#547, #582, #583, #597, #598, #603, #605).
