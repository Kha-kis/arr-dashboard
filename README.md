...
> **Version 2.19.0** — Continues the #427 OOM mitigation arc with stream-parsing the bulk library-sync JSON response, stream-fetching the hunting catalog, slimming wanted/movie/album/book records, and adaptive concurrency on library-sync (#448, #449, #451, #452, #453, #456). Adds Seerr `Test Connection` permission-probing and surfaces upstream Seerr errors inline on the Discover page (closes #465). Adds explanatory tooltip + legend to Settings → System → Schema Drift (closes #455). Adds TRaSH migration notices for the upstream German/French unwanted-format group splits (#2719/#2721). Removes the legacy TRaSH NAMING fetcher whose dead-code path was producing two false-positive validation warnings per cache refresh. Plus dependency security bumps, hunting scheduler reentrancy fix (#457), and new diagnostic tooling (heap retainer-walk, dump-heap helper, auto-snapshot at 90%).
