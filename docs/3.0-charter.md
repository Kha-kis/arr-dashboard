...
- **Prior beta removal proven by the existing migration report:** show a one-time recovery notice linking to the add-Tautulli form and recovery documentation; absence alone is not proof and credentials or rules are never reconstructed from stale data.
...
- Manual smoke test: fresh install plus Tautulli-only, Tracearr-only, both-configured, neither-configured, and proven-prior-removal provider states
