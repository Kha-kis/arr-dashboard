...
Tracearr is the recommended/default historical analytics provider, and Tautulli is a supported alternative. Plex / Jellyfin / Emby continue to provide live-session data via native APIs; the selected analytics provider provides historical analytics, leaderboards, account-sharing detection, and operator actions. Provider families are never mixed and never silently fail over.
...
- Provider selection and non-blocking migration notices (§6.1) share the same component family
...
| # | Change | Rationale |
|---|---|---|
| A1 | **Route-manifest tier reshuffle.** Auto-Tagger / Label Sync / Hunting / Queue Cleaner / TRaSH Guides promote per ADR-0005. Tracearr and Tautulli provider surfaces follow their approved tiers. qui (and Cross-seed) promote `experimental` → `stable`. | Tiers diverged from maturity; honest tiers make tier-based decisions safe. |
| A2 | **Analytics-provider governance.** Preserve Tautulli as a supported alternative, make Tracearr recommended/default, and use deterministic selection with no mixing or silent failover. Existing Tautulli state remains intact; migration notices are non-blocking. | Provider choice must preserve operator control and avoid destructive upgrades. |
...
Historical record of the superseded removal decision. See ADR-0009 for the
approved Tracearr-primary/Tautulli-alternative provider model.

### 5.4 ADR-0009 — Tracearr Primary with Tautulli Alternative
Records deterministic provider selection, preservation semantics, non-blocking
migration notices, no mixing or failover, and identity-gated cleanup evidence.

### 5.5 ADR-0008 — Setup Auto-Discovery Scope
...
### 6.1 Analytics-provider preservation and selection

There is no blocking removal wizard. `analyticsProvider` accepts `tracearr` or
`tautulli`; fresh installations default to Tracearr, Tautulli-only upgrades
select Tautulli, and upgrades with both providers select Tracearr unless an
explicit setting already exists.

- **Tautulli only:** preserve every instance and rule, select Tautulli, and continue without prompting.
- **Tracearr only:** keep Tracearr selected and continue without prompting.
- **Both configured:** select Tracearr by default and show a one-time, non-blocking notice linking to the provider selector.
- **Neither configured:** continue normally with setup links for both providers.
- **Prior beta removal:** the existing installation-wide migration report is
  historical audit evidence only; it has no user or deleted-instance identity
  and cannot prove user-scoped removal. Current upgrades show no recovery or
  removal notice based solely on that report.

The `tautulli-prior-removal` notice kind remains reserved and dormant until a
future durable evidence source identifies the affected user and actual deletion.
The notice is dismissible and durable. It never deletes data. Historical
analytics come from the selected provider family only; an outage reports
degraded analytics without switching providers. Identity-dependent cleanup
evidence fails closed until the upstream identity is enrolled and matches
...
- Manual smoke test: fresh install plus Tautulli-only, Tracearr-only, both-configured, neither-configured, and the reserved/dormant prior-removal state
...
- Provider-preservation notices produce **zero** support reports about lost Tautulli configuration or rules for 4 weeks post-release
...
- **`tautulli-deprecation-plan.md`.** Superseded by this charter and ADR-0009. Add a STATUS notice pointing readers at `docs/3.0-charter.md` §6.1; update MEMORY.md description accordingly.
...
| 2 | Analytics providers | Tracearr recommended/default; Tautulli supported alternative |
| 3 | 2.21 warning banner | Skip entirely |
| 4 | Migration behavior | Non-blocking, durable notices |
| 5 | Provider selection | Deterministic, single-family, no failover |
