# Historical 3.0 v2.21 SQLite Upgrade Rehearsal (superseded)
...
This records the former beta-entry upgrade rehearsal required by the
[3.0 release-readiness audit](3.0-release-readiness.md). It used a preserved
SQLite fixture with realistic Tautulli removal state, ran the actual combined
container and schema synchronization, completed the blocking browser dialog,
and restarted the container to verify idempotence.

> **Superseded historical evidence:** This rehearsal tested the former
> destructive Tautulli removal path. ADR-0009 and Wave 4A preserve Tautulli
> configuration and rules and use non-blocking provider notices instead. Its
> pass result is not current B5 evidence. B5 is reopened pending a fresh,
> data-bearing preservation rehearsal that proves retained configuration and
> rules plus non-blocking notice behavior.
