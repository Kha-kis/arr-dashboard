...
- **Reverse-proxy HTTP Basic Auth** — configure it in the dedicated
  ...
  both connection tests and runtime calls. Tracearr and Jellyfin are
  excluded because their modern API authentication already occupies the
  `Authorization` header; configure a proxy bypass for arr-dashboard.
  Prefer HTTPS because Basic credentials are only encoded, not encrypted,
