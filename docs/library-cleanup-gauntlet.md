...
| LC-618 | [#618](https://github.com/Kha-kis/arr-dashboard/issues/618): rules need nested AND/OR groups and NOT | Round-trip a versioned recursive rule tree through API, database, editor, preview, explain, approval, and execution; prove backward compatibility for existing composite rules | The recursive policy/provider foundation is independently clean across schema, evaluator, API, editor, preview/explain, current-policy authority, provider freshness, and legacy compatibility. Integration with the remaining cleanup slices and the live browser gate remain open |
| LC-619 | [#619](https://github.com/Kha-kis/arr-dashboard/issues/619): monitored items need a direct condition | Prove monitored and unmonitored matches in standalone, nested, retention, preview, explain, and execution paths | Independently clean in the integrated foundation across standalone/nested/NOT evaluation, explain, auto-tag, queued/direct authority, missing-evidence failure, and timeout reconciliation. Preview-slice integration and live browser gates remain open |
...
Open-issue and comment audit refreshed 2026-08-04: the repository's complete open Library
Cleanup set is #616, #618, #619, #657, #660, and #667, all represented above. Closed
#617 and #659 remain mandatory regression gates because their failure modes are part of
...
| --- | --- | --- | --- |
| RULE-001 `(A AND B) OR (A AND C)` | Only the mathematically matching items are proposed and executed | Shared schema/evaluator tests, API round-trip, editor E2E | Planned |
| RULE-002 `A AND NOT B` | Negation is explicit and identical in preview/explain/execution | Property and truth-table tests, editor E2E | Planned |
| RULE-003 monitored/unmonitored | Each state can be selected directly without retention-rule workarounds | API/evaluator/UI tests | Flat-rule automated and authority gates green (full repository tests, lint, typecheck, and build); nested-rule and live gate open |
