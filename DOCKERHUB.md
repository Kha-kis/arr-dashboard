...
> **Version 2.21.0** — The **media-only storage & auth resilience release**. The combined Storage figure now filters to disks holding configured *arr root folders — container `/` and config volumes no longer inflate the total — with a new expandable **storage breakdown panel** showing which disks were counted, deduplicated, or excluded and why (#504, #505, closes #495). Works on containerized, bare-metal Linux, and Windows-native deployments; degrades safely with no root folders configured. A misconfigured OIDC provider can no longer lock you out: password, passkey, and OIDC auth now coexist (#501, closes #498). Tautulli sparse-metadata responses no longer flood Dashboard/Pulse with validation warnings (#502, closes #497). Security: `hono` override → 4.12.21, closing CVE-2026-47673…47676 (#503). Plus dependency sweeps (#499, #500).
