...
> **Version 2.20.0** — The **qui integration release**. Federates with [autobrr/qui](https://github.com/autobrr/qui) for torrent-layer observability across the library, Pulse, queue cleaner, and a new `/qui` overview: per-card torrent health and tracker brand icons, server-side torrent-state filter, sort-by-ratio, a 480px capability-aware detail drawer, MediaInfo quality verification, season-grouped torrent panels on series/movies, queue-cleaner last-seed protection, and a qui Activity tab with its own webhook receiver (#475). Reframes cross-seed as an **integrity lens** keyed on tracker health (#492). Two privacy fixes strip raw tracker passkeys from the qui trackers route (#493, closes #491) and the cross-seed display (#492). Storage Available card de-duplicates shared disks and shows "N disks across M instances" (#490, closes #486). Library Cleanup remembers rejected items (#482, closes #474). Hunting grab detection without `eventType` filter (#479, closes #472). `MALLOC_ARENA_MAX=2` container default + rss/heap ratio (#478). Plus query-string percent-encoding (#476, closes #470), heap-monitor kill switch (#477, closes #471), CodeQL qui debug env-gate (#485), and dependency sweeps.
