...
> **Version 2.19.0** — Continues the #427 OOM mitigation arc (stream-parsing library-sync JSON, stream-fetching hunting catalog, slim wanted/movie/album/book records, adaptive concurrency). Adds Seerr `Test Connection` permission-probing and surfaces upstream Seerr errors inline on Discover (closes #465). Adds Schema Drift help tooltip + legend in Settings → System (closes #455). Adds TRaSH migration notices for upstream German/French unwanted-format group splits. Removes the legacy TRaSH NAMING fetcher whose dead-code path produced false-positive validation warnings on every cache refresh. Plus dependency security bumps, hunting scheduler reentrancy fix (#457), and new diagnostic tooling.
